...
monsterDoor.addEventListener('click', () => {
    if(monsterDoor.dataset.door === 'close'){
        ...
        for(let i = 0; i <= dungeonLevel; i++){
            ...
            if(rollRandomResult(3) == 2) {
                monster.setAttribute('data-class','specialist')
            }
            if(rollRandomResult(3) == 3) {
                monster.setAttribute('data-class','divine')
            }
            if(rollRandomResult(3) == 4) {
                monster.setAttribute('data-class','arcane')
            }
            // cola na tela
